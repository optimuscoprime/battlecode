package bbSwarmImproved3_ninja;
/*
			...
			try{
               ...
               int beGreaterBy= rc.hasUpgrade(Upgrade.FUSION) ? SUPERIORITY*2:SUPERIORITY;
					...
					if(((rc.getTeamPower()-40>10)||(Clock.getRoundNum()<2))&&(numFriendlies < (numEnemies +beGreaterBy))){
                     ...
                     Direction.NONE)&&(rc.senseMine(rc.getLocation().add(d)) ==
                     null)&&((rc.senseMine(rc.getLocation().add(d)) ==
                     null)||(rc.senseMine(rc.getLocation().add(d)) == rc.getTeam())) ){
								rc.spawn(d);
   ...
   private static void scoutCode(){
      ...
      while(true){
         try{
	       ...
	       // if you can move where you want to, them move there
	       if (rc.canMove(dir)) {
	       ...
	       // else check one to the left and right
	       ...
	       // if there's mines inf front, to the left and right, then check a little more
	       ...
	       int dir_dist_to_enemy_hq = location_in_dir.distanceSquaredTo(enemy_hq);
		   ...
		   if (dir_dist_to_enemy_hq_left_left < dir_dist_to_enemy_hq) {
		       if (rc.canMove(dir_left_left)) {
			   rc.move(dir_left_left);
			   break;
		       }
		   } else if (dir_dist_to_enemy_hq_right_right < dir_dist_to_enemy_hq) {
